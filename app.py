...
class User(db.Model):
    ...
    trusted_contacts = db.Column(db.Text)
...
class Volunteer(db.Model):
    ...
    latitude = db.Column(db.Float)
    longitude = db.Column(db.Float)
    service_radius = db.Column(db.Float, default=5.0)
...
def volunteer_signup():
    if request.method == 'POST':
        ...
        volunteer = Volunteer(
            ...
            phone=phone, address=address, service_radius=service_radius
        )
...
def sos_alert():
    ...
    contacts = []
    if user.trusted_contacts and twilio_number:
        ...
        print("⚠️ No trusted contacts or Twilio number not set.")
